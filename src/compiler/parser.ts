...
module ts {
    ...
    export function createNode(kind: SyntaxKind): Node { return new (getNodeConstructor(kind))(); }
    ...
    export function forEachChild<T>(node: Node, cbNode: (node: Node) => T, cbNodeArray ?: (nodes: Node[]) => T): T {
        ...
        switch (node.kind) {
            ...
            case SyntaxKind.CatchClause:
                return visitNode(cbNode, (<CatchClause> node).name) || visitNode(cbNode, (<CatchClause> node).type) ||
                       visitNode(cbNode, (<CatchClause> node).block);
            case SyntaxKind.ClassDeclaration:
                return visitNodes(cbNodes, node.modifiers) || visitNode(cbNode, (<ClassDeclaration> node).name) ||
            ...
            case SyntaxKind.InterfaceDeclaration:
                return visitNodes(cbNodes, node.modifiers) || visitNode(cbNode, (<InterfaceDeclaration> node).name) ||
                       visitNodes(cbNodes, (<InterfaceDeclaration> node).typeParameters) ||
                       visitNodes(cbNodes, (<ClassDeclaration> node).heritageClauses) ||
                       visitNodes(cbNodes, (<InterfaceDeclaration> node).members);
            case SyntaxKind.TypeAliasDeclaration:
                return visitNodes(cbNodes, node.modifiers) || visitNode(cbNode, (<TypeAliasDeclaration> node).name) ||
                       visitNode(cbNode, (<TypeAliasDeclaration> node).type);
            case SyntaxKind.EnumDeclaration:
                return visitNodes(cbNodes, node.modifiers) || visitNode(cbNode, (<EnumDeclaration> node).name) ||
            ...
            case SyntaxKind.ModuleDeclaration:
                return visitNodes(cbNodes, node.modifiers) || visitNode(cbNode, (<ModuleDeclaration> node).name) ||
            ...
            case SyntaxKind.ExportAssignment:
                return visitNodes(cbNodes, node.modifiers) || visitNode(cbNode, (<ExportAssignment> node).exportName);
            case SyntaxKind.TemplateExpression:
                return visitNode(cbNode, (<TemplateExpression> node).head) ||
                       visitNodes(cbNodes, (<TemplateExpression> node).templateSpans);
            case SyntaxKind.TemplateSpan:
                return visitNode(cbNode, (<TemplateSpan> node).expression) ||
                       visitNode(cbNode, (<TemplateSpan> node).literal);
    ...
    function parsingContextErrors(context: ParsingContext): DiagnosticMessage {
        switch (context) {
            ...
            case ParsingContext.TypeReferences:
                return Diagnostics.Type_reference_expected;
    ...
    function moveElementEntirelyPastChangeRange(element: IncrementalElement, isArray: boolean, delta: number,
                                                oldText: string, newText: string, aggressiveChecks: boolean) {
        if (isArray) {
    ...
    function adjustIntersectingElement(element: IncrementalElement, changeStart: number, changeRangeOldEnd: number,
                                       changeRangeNewEnd: number, delta: number) {
    ...
    function updateTokenPositionsAndMarkElements(sourceFile: IncrementalNode, changeStart: number,
                                                 changeRangeOldEnd: number, changeRangeNewEnd: number, delta: number,
                                                 oldText: string, newText: string, aggressiveChecks: boolean): void {
        ...
        function visitNode(child: IncrementalNode) {
            ...
            if (child.pos > changeRangeOldEnd) {
                ...
                moveElementEntirelyPastChangeRange(child, /*isArray:*/ false, delta, oldText, newText,
                                                   aggressiveChecks);
                return;
            }
    ...
    function checkChangeRange(sourceFile: SourceFile, newText: string, textChangeRange: TextChangeRange,
    ...
    export function updateSourceFile(sourceFile: SourceFile, newText: string, textChangeRange: TextChangeRange,
        ...
        if (sourceFile.statements.length === 0) {
            ...
            return parseSourceFile(sourceFile.fileName, newText, sourceFile.languageVersion, /*syntaxCursor*/ undefined,
                                   /*setNodeParents*/ true)
        }
        ...
        Debug.assert(textSpanEnd(textChangeRangeNewSpan(changeRange)) ===
                     textSpanEnd(textChangeRangeNewSpan(textChangeRange)));
...
export function createSourceFile(fileName: string, sourceText: string, languageVersion: ScriptTarget,
                                 setParentNodes = false): SourceFile {
...
function parseSourceFile(fileName: string, sourceText: string, languageVersion: ScriptTarget,
                         syntaxCursor: SyntaxCursor, setParentNodes = false): SourceFile {
    ...
    function setStrictModeContext(val: boolean) { setContextFlag(val, ParserContextFlags.StrictMode); }

    function setDisallowInContext(val: boolean) { setContextFlag(val, ParserContextFlags.DisallowIn); }

    function setYieldContext(val: boolean) { setContextFlag(val, ParserContextFlags.Yield); }

    function setGeneratorParameterContext(val: boolean) { setContextFlag(val, ParserContextFlags.GeneratorParameter); }
    ...
    function inYieldContext() { return (contextFlags & ParserContextFlags.Yield) !== 0; }

    function inStrictModeContext() { return (contextFlags & ParserContextFlags.StrictMode) !== 0; }

    function inGeneratorParameterContext() { return (contextFlags & ParserContextFlags.GeneratorParameter) !== 0; }

    function inDisallowInContext() { return (contextFlags & ParserContextFlags.DisallowIn) !== 0; }
    ...
    function nextToken(): SyntaxKind { return token = scanner.scan(); }

    function getTokenPos(pos: number): number { return skipTrivia(sourceText, pos); }

    function reScanGreaterToken(): SyntaxKind { return token = scanner.reScanGreaterToken(); }

    function reScanSlashToken(): SyntaxKind { return token = scanner.reScanSlashToken(); }

    function reScanTemplateToken(): SyntaxKind { return token = scanner.reScanTemplateToken(); }
    ...
    function lookAhead<T>(callback: () => T): T { return speculationHelper(callback, /*isLookAhead:*/ true); }

    // Invokes the provided callback.  If the callback returns something falsy, then it restores
    // the parser to the state it was in immediately prior to invoking the callback.  If the
    // callback returns something truthy, then the parser state is not rolled back.  The result
    // of invoking the callback is returned from this function.
    function tryParse<T>(callback: () => T): T { return speculationHelper(callback, /*isLookAhead:*/ false); }
    ...
    function createMissingNode(kind: SyntaxKind, reportAtCurrentPosition: boolean, diagnosticMessage: DiagnosticMessage,
                               arg0 ?: any): Node {
    ...
    function createIdentifier(isIdentifier: boolean, diagnosticMessage ?: DiagnosticMessage): Identifier {
        ...
        return <Identifier> createMissingNode(SyntaxKind.Identifier, /*reportAtCurrentPosition:*/ false,
                                              diagnosticMessage || Diagnostics.Identifier_expected);
    }
    ...
    function parseIdentifierName(): Identifier { return createIdentifier(isIdentifierOrKeyword()); }

    function isLiteralPropertyName(): boolean {
        return isIdentifierOrKeyword() || token === SyntaxKind.StringLiteral || token === SyntaxKind.NumericLiteral;
    }
    ...
    function canFollowModifier(): boolean {
        return token === SyntaxKind.OpenBracketToken || token === SyntaxKind.OpenBraceToken ||
               token === SyntaxKind.AsteriskToken || isLiteralPropertyName();
    }
    ...
    function isListElement(parsingContext: ParsingContext, inErrorRecovery: boolean): boolean {
        ...
        switch (parsingContext) {
            ...
            case ParsingContext.ObjectLiteralMembers:
                return token === SyntaxKind.OpenBracketToken || token === SyntaxKind.AsteriskToken ||
    ...
    function parseList<T extends Node>(kind: ParsingContext, checkForStrictMode: boolean,
                                       parseElement: () => T): NodeArray<T> {
    ...
    function isReusableEnumMember(node: Node) { return node.kind === SyntaxKind.EnumMember; }
    ...
    function parseDelimitedList<T extends Node>(kind: ParsingContext, parseElement: () => T,
                                                considerSemicolonAsDelimeter ?: boolean): NodeArray<T> {
        var saveParsingContext = parsingContext;
        parsingContext |= 1 << kind;
        var result = <NodeArray<T>>[];
        result.pos = getNodePos();
        ...
        while (true) {
            if (isListElement(kind, /* inErrorRecovery */ false)) {
                ...
                if (considerSemicolonAsDelimeter && token === SyntaxKind.SemicolonToken &&
                    !scanner.hasPrecedingLineBreak()) {
                    nextToken();
                }
                continue;
    ...
    function parseBracketedList<T extends Node>(kind: ParsingContext, parseElement: () => T, open: SyntaxKind,
                                                close: SyntaxKind): NodeArray<T> {
    ...
    function parseRightSideOfDot(allowIdentifierNames: boolean): Identifier {
        ...
        if (scanner.hasPrecedingLineBreak() && scanner.isReservedWord()) {
            ...
            if (matchesPattern) {
                ...
                return <Identifier> createMissingNode(SyntaxKind.Identifier, /*reportAtCurrentToken:*/ true,
    ...
    function parseTemplateSpan(): TemplateSpan {
        ...
        if (token === SyntaxKind.CloseBraceToken) {
            reScanTemplateToken() literal = parseLiteralNode();
        } else {
            literal = <LiteralExpression> createMissingNode(SyntaxKind.TemplateTail, /*reportAtCurrentPosition:*/ false,
    ...
    function parseLiteralNode(internName ?: boolean): LiteralExpression {
        ...
        if (node.kind === SyntaxKind.NumericLiteral && sourceText.charCodeAt(tokenPos) === CharacterCodes._0 &&
    ...
    function parseTypeReference(): TypeReferenceNode {
        ...
        if (!scanner.hasPrecedingLineBreak() && token === SyntaxKind.LessThanToken) {
            node.typeArguments = parseBracketedList(ParsingContext.TypeArguments, parseType, SyntaxKind.LessThanToken,
    ...
    function parseTypeParameters(): NodeArray<TypeParameterDeclaration> {
        if (token === SyntaxKind.LessThanToken) {
            return parseBracketedList(ParsingContext.TypeParameters, parseTypeParameter, SyntaxKind.LessThanToken,
                                      SyntaxKind.GreaterThanToken);
        }
    }
    ...
    function parseParameterType(): TypeNode {
        if (parseOptional(SyntaxKind.ColonToken)) {
            return token === SyntaxKind.StringLiteral ? <StringLiteralTypeNode> parseLiteralNode(/*internName:*/ true) :
                                                        parseType();
        }
    ...
    function parseParameter(): ParameterDeclaration {
        ...
        if (getFullWidth(node.name) === 0 && node.flags === 0 && isModifier(token)) {
            ...
            // so result of this function will be ParameterDeclaration (flags = 0, name = missing, type = undefined,
            // initializer = undefined)
            // and current token will not change => parsing of the enclosing parameter list will last till the end of
            // time (or OOM)
        ...
        node.initializer = inGeneratorParameterContext() ? doOutsideOfYieldContext(parseParameterInitializer) :
    ...
    function parseParameterInitializer() { return parseInitializer(/*inParameter*/ true); }

    function fillSignature(returnToken: SyntaxKind, yieldAndGeneratorParameterContext: boolean,
                           requireCompleteParameterList: boolean, signature: SignatureDeclaration): void {
    ...
    function parseSignatureMember(kind: SyntaxKind): SignatureDeclaration {
        ...
        fillSignature(SyntaxKind.ColonToken, /*yieldAndGeneratorParameterContext:*/ false,
                      /*requireCompleteParameterList:*/ false, node);
    ...
    function parseIndexSignatureDeclaration(modifiers: ModifiersArray): IndexSignatureDeclaration {
        ...
        node.parameters = parseBracketedList(ParsingContext.Parameters, parseParameter, SyntaxKind.OpenBracketToken,
    ...
    function parsePropertyOrMethodSignature(): Declaration {
        ...
        if (token === SyntaxKind.OpenParenToken || token === SyntaxKind.LessThanToken) {
            ...
            fillSignature(SyntaxKind.ColonToken, /*yieldAndGeneratorParameterContext:*/ false,
                          /*requireCompleteParameterList:*/ false, method);
            parseTypeMemberSemicolon();
    ...
    function isTypeMemberWithLiteralPropertyName() {
        ...
        return token === SyntaxKind.OpenParenToken || token === SyntaxKind.LessThanToken ||
               token === SyntaxKind.QuestionToken || token === SyntaxKind.ColonToken || canParseSemicolon();
    }
    ...
    function parseTupleType(): TupleTypeNode {
        ...
        node.elementTypes = parseBracketedList(ParsingContext.TupleElementTypes, parseType, SyntaxKind.OpenBracketToken,
                                               SyntaxKind.CloseBracketToken);
        return finishNode(node);
    }
    ...
    function parseFunctionOrConstructorType(kind: SyntaxKind): FunctionOrConstructorTypeNode {
        ...
        fillSignature(SyntaxKind.EqualsGreaterThanToken, /*yieldAndGeneratorParameterContext:*/ false,
                      /*requireCompleteParameterList:*/ false, node);
        return finishNode(node);
    }
    ...
    function isUnambiguouslyStartOfFunctionType() {
        ...
        if (isIdentifier() || isModifier(token)) {
            nextToken();
            if (token === SyntaxKind.ColonToken || token === SyntaxKind.CommaToken ||
                token === SyntaxKind.QuestionToken || token === SyntaxKind.EqualsToken || isIdentifier() ||
                isModifier(token)) {
                // ( id :
                // ( id ,
                // ( id ?
    ...
    function parseInitializer(inParameter: boolean): Expression {
        if (token !== SyntaxKind.EqualsToken) {
            ...
            if (scanner.hasPrecedingLineBreak() || (inParameter && token === SyntaxKind.OpenBraceToken) ||
                !isStartOfExpression()) {
                // preceding line break, open brace in a parameter (likely a function body) or current token is not an
                // expression -
    ...
    function parseParenthesizedArrowFunctionExpressionHead(allowAmbiguity: boolean): FunctionExpression {
        ...
        // close paren.
        fillSignature(SyntaxKind.ColonToken, /*yieldAndGeneratorParameterContext:*/ false,
                      /*requireCompleteParameterList:*/ !allowAmbiguity, node);
    ...
    function parseArrowFunctionExpressionBody(): Block | Expression {
        ...
        if (isStartOfStatement(/*inErrorRecovery:*/ true) && !isStartOfExpressionStatement() &&
    ...
    function isInOrOfKeyword(t: SyntaxKind) { return t === SyntaxKind.InKeyword || t === SyntaxKind.OfKeyword; }
    ...
    function parseLeftHandSideExpressionOrHigher(): LeftHandSideExpression {
        ...
        var expression = token === SyntaxKind.SuperKeyword ? parseSuperExpression() : parseMemberExpressionOrHigher();
    ...
    function parseArgumentOrArrayLiteralElement(): Expression {
        return token === SyntaxKind.DotDotDotToken ? parseSpreadElement() : token === SyntaxKind.CommaToken ?
                                                     <Expression> createNode(SyntaxKind.OmittedExpression) :
                                                     parseAssignmentExpressionOrHigher();
    }

    function parseArgumentExpression(): Expression { return allowInAnd(parseArgumentOrArrayLiteralElement); }
    ...
    function parseFunctionExpression(): FunctionExpression {
        ...
        fillSignature(SyntaxKind.ColonToken, /*yieldAndGeneratorParameterContext:*/ !!node.asteriskToken,
                      /*requireCompleteParameterList:*/ false, node);
    ...
    // STATEMENTS
    function parseBlock(ignoreMissingOpenBrace: boolean, checkForStrictMode: boolean,
                        diagnosticMessage ?: DiagnosticMessage): Block {
    ...
    function parseFunctionBlock(allowYield: boolean, ignoreMissingOpenBrace: boolean,
                                diagnosticMessage ?: DiagnosticMessage): Block {
        var savedYieldContext = inYieldContext();
    ...
    function parseVariableStatementOrFunctionDeclarationWithModifiers(): FunctionDeclaration | VariableStatement {
        ...
        switch (token) {
            case SyntaxKind.ConstKeyword:
                var nextTokenIsEnum = lookAhead(nextTokenIsEnumKeyword) if (nextTokenIsEnum) { return undefined; }
    ...
    function parseMethodDeclaration(fullStart: number, modifiers: ModifiersArray, asteriskToken: Node,
                                    name: DeclarationName, questionToken: Node,
                                    diagnosticMessage ?: DiagnosticMessage): MethodDeclaration {
        var method = <MethodDeclaration> createNode(SyntaxKind.MethodDeclaration, fullStart);
        setModifiers(method, modifiers);
        method.asteriskToken = asteriskToken;
        method.name = name;
        method.questionToken = questionToken;
        fillSignature(SyntaxKind.ColonToken, /*yieldAndGeneratorParameterContext:*/ !!asteriskToken,
                      /*requireCompleteParameterList:*/ false, method);
    ...
    function parsePropertyOrMethodDeclaration(fullStart: number, modifiers: ModifiersArray): ClassElement {
        ...
        var questionToken = parseOptionalToken(SyntaxKind.QuestionToken);
        if (asteriskToken || token === SyntaxKind.OpenParenToken || token === SyntaxKind.LessThanToken) {
            return parseMethodDeclaration(fullStart, modifiers, asteriskToken, name, questionToken,
    ...
    function parseAccessorDeclaration(kind: SyntaxKind, fullStart: number,
                                      modifiers: ModifiersArray): AccessorDeclaration {
    ...
    function parseInternalModuleTail(fullStart: number, modifiers: ModifiersArray,
    ...
    function nextTokenIsOpenParen() { return nextToken() === SyntaxKind.OpenParenToken; }
    ...
    function parseModuleReference() {
        return isExternalModuleReference() ? parseExternalModuleReference() :
    ...
    function parseExportSpecifier() { return parseImportOrExportSpecifier(SyntaxKind.ExportSpecifier); }

    function parseImportSpecifier() { return parseImportOrExportSpecifier(SyntaxKind.ImportSpecifier); }
    ...
    function isIdentifierOrKeyword() { return token >= SyntaxKind.Identifier; }
    ...
    function nextTokenCanFollowImportKeyword() {
        nextToken();
        return isIdentifierOrKeyword() || token === SyntaxKind.StringLiteral || token === SyntaxKind.AsteriskToken ||
    ...
    function parseSourceElement() { return parseSourceElementOrModuleElement(); }
