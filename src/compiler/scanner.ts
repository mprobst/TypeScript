...
module ts {
    export interface ErrorCallback { (message: DiagnosticMessage, length: number): void; }
    ...
    */
    ...
    /* @internal */ export function isUnicodeIdentifierStart(code: number, languageVersion: ScriptTarget) {
        return languageVersion >= ScriptTarget.ES5 ? lookupInUnicodeMap(code, unicodeES5IdentifierStart) :
                                                     lookupInUnicodeMap(code, unicodeES3IdentifierStart);
    }

    function isUnicodeIdentifierPart(code: number, languageVersion: ScriptTarget) {
        return languageVersion >= ScriptTarget.ES5 ? lookupInUnicodeMap(code, unicodeES5IdentifierPart) :
    ...
    export function tokenToString(t: SyntaxKind): string { return tokenStrings[t]; }
    ...
    export function isWhiteSpace(ch: number): boolean {
        return ch === CharacterCodes.space || ch === CharacterCodes.tab || ch === CharacterCodes.verticalTab ||
               ch === CharacterCodes.formFeed || ch === CharacterCodes.nonBreakingSpace ||
               ch === CharacterCodes.ogham || ch >= CharacterCodes.enQuad && ch <= CharacterCodes.zeroWidthSpace ||
               ch === CharacterCodes.narrowNoBreakSpace || ch === CharacterCodes.mathematicalSpace ||
               ch === CharacterCodes.ideographicSpace || ch === CharacterCodes.byteOrderMark;
    }
    ...
    function isDigit(ch: number): boolean { return ch >= CharacterCodes._0 && ch <= CharacterCodes._9; }

    export function isOctalDigit(ch: number): boolean { return ch >= CharacterCodes._0 && ch <= CharacterCodes._7; }
    ...
    // Extract comments from the given source text starting at the given position. If trailing is false, whitespace is
    // skipped until
    // the first line break and comments between that location and the next token are returned. If trailing is true,
    // comments occurring
    // between the given position and the next line break are returned. The return value is an array containing a
    // TextRange for each
    // comment. Single-line comment ranges include the beginning '//' characters but not the ending line break.
    // Multi-line comment
    // ranges include the beginning '/* and ending '*/' characters. The return value is undefined if no comments were
    // found.
    ...
    export function createScanner(languageVersion: ScriptTarget, skipTrivia: boolean, text ?: string,
        ...
        function isIdentifierStart(ch: number): boolean {
            return ch >= CharacterCodes.A && ch <= CharacterCodes.Z ||
                   ch >= CharacterCodes.a && ch <= CharacterCodes.z || ch === CharacterCodes.$ ||
                   ch === CharacterCodes._ ||
                   ch > CharacterCodes.maxAsciiCharacter && isUnicodeIdentifierStart(ch, languageVersion);
        }
        ...
        function scanTemplateAndSetTokenValue(): SyntaxKind {
            ...
            pos++;
            var start = pos;
            var contents = "" var resultingToken: SyntaxKind;
            ...
            while (true) {
                ...
                // '${'
                if (currChar === CharacterCodes.$ && pos + 1 < len &&
        ...
        function scanEscapeSequence(): string {
            ...
            switch (ch) {
                ...
                case CharacterCodes.paragraphSeparator:
                    return "" default: return String.fromCharCode(ch);
            }
        }
        ...
        function scanBinaryOrOctalDigits(base: number): number {
            ...
            var value = 0;
            // For counting number of digits; Valid binaryIntegerLiteral must have at least one binary digit following B
            // or b.
                ...
                switch (ch) {
                    ...
                    case CharacterCodes.carriageReturn:
                        ...
                        if (skipTrivia) {
                            ...
                            if (ch === CharacterCodes.carriageReturn && pos + 1 < len &&
                    ...
                    case CharacterCodes._0:
                        if (pos + 2 < len && (text.charCodeAt(pos + 1) === CharacterCodes.X ||
                            ...
                            tokenValue = "" + value;
                            return token = SyntaxKind.NumericLiteral;
                        } else if (pos + 2 < len && (text.charCodeAt(pos + 1) === CharacterCodes.B ||
                            ...
                            tokenValue = "" + value;
                            return token = SyntaxKind.NumericLiteral;
                        } else if (pos + 2 < len && (text.charCodeAt(pos + 1) === CharacterCodes.O ||
                    ...
                    // This fall-through is a deviation from the EcmaScript grammar. The grammar says that a leading
                    // zero
                    // can only be followed by an octal digit, a dot, or the end of the number literal. However, we are
                    // being
        ...
        function reScanSlashToken(): SyntaxKind {
            if (token === SyntaxKind.SlashToken || token === SyntaxKind.SlashEqualsToken) {
                ...
                while (true) {
                    ...
                    if (p >= len) {
                        tokenIsUnterminated = true;
                        error(Diagnostics.Unterminated_regular_expression_literal) break;
                    }
        ...
        function tryScan<T>(callback: () => T): T { return speculationHelper(callback, /*isLookahead:*/ false); }
