...
module ts {
    ...
    function bindSourceFileWorker(file: SourceFile): void {
        ...
        function declareSymbol(symbols: SymbolTable, parent: Symbol, node: Declaration, includes: SymbolFlags,
        ...
        function declareModuleMember(node: Declaration, symbolKind: SymbolFlags, symbolExcludes: SymbolFlags) {
            ...
            if (symbolKind & SymbolFlags.Import) {
                ...
                //      when the emitter comes back to it, it knows not to qualify the name if it was found in a
                //      containing scope.
        ...
        // All container nodes are kept on a linked list in declaration order. This list is used by the
        // getLocalNameOfContainer function
        ...
        function bindAnonymousDeclaration(node: Declaration, symbolKind: SymbolFlags, name: string,
                                          isBlockScopeContainer: boolean) {
        ...
        function bindBlockScopedVariableDeclaration(node: Declaration) {
            switch (blockScopeContainer.kind) {
                ...
                case SyntaxKind.SourceFile:
                    if (isExternalModule(<SourceFile> container)) {
                        declareModuleMember(node, SymbolFlags.BlockScopedVariable,
                ...
                default:
                    ...
                    declareSymbol(blockScopeContainer.locals, undefined, node, SymbolFlags.BlockScopedVariable,
                                  SymbolFlags.BlockScopedVariableExcludes);
            }
            ...
            switch (node.kind) {
                ...
                case SyntaxKind.BindingElement:
                    if (isBindingPattern((<Declaration> node).name)) {
                        ...
                        bindDeclaration(<Declaration> node, SymbolFlags.FunctionScopedVariable,
                                        SymbolFlags.FunctionScopedVariableExcludes, /*isBlockScopeContainer*/ false);
                    }
                    break;
                ...
                case SyntaxKind.ShorthandPropertyAssignment:
                    bindPropertyOrMethodOrAccessor(<Declaration> node, SymbolFlags.Property,
                                                   SymbolFlags.PropertyExcludes, /*isBlockScopeContainer*/ false);
                    break;
                case SyntaxKind.EnumMember:
                    bindPropertyOrMethodOrAccessor(<Declaration> node, SymbolFlags.EnumMember,
                                                   SymbolFlags.EnumMemberExcludes, /*isBlockScopeContainer*/ false);
                    break;
                case SyntaxKind.CallSignature:
                case SyntaxKind.ConstructSignature:
                case SyntaxKind.IndexSignature:
                ...
                case SyntaxKind.FunctionDeclaration:
                    ...
                    break;
                case SyntaxKind.Constructor:
                    bindDeclaration(<Declaration> node, SymbolFlags.Constructor, /*symbolExcludes:*/ 0,
                                    /*isBlockScopeContainer:*/ true);
                    break;
                case SyntaxKind.GetAccessor:
                    bindPropertyOrMethodOrAccessor(<Declaration> node, SymbolFlags.GetAccessor,
                                                   SymbolFlags.GetAccessorExcludes, /*isBlockScopeContainer*/ true);
                    break;
                case SyntaxKind.SetAccessor:
                    bindPropertyOrMethodOrAccessor(<Declaration> node, SymbolFlags.SetAccessor,
                                                   SymbolFlags.SetAccessorExcludes, /*isBlockScopeContainer*/ true);
                    break;
                ...
                case SyntaxKind.EnumDeclaration:
                    if (isConst(node)) {
                        bindDeclaration(<Declaration> node, SymbolFlags.ConstEnum, SymbolFlags.ConstEnumExcludes,
                                        /*isBlockScopeContainer*/ false);
                    } else {
                        bindDeclaration(<Declaration> node, SymbolFlags.RegularEnum, SymbolFlags.RegularEnumExcludes,
                                        /*isBlockScopeContainer*/ false);
                    }
                    break;
                case SyntaxKind.ModuleDeclaration:
                ...
                case SyntaxKind.SourceFile:
                    if (isExternalModule(<SourceFile> node)) {
                        bindAnonymousDeclaration(<SourceFile> node, SymbolFlags.ValueModule,
                                                 '"' + removeFileExtension((<SourceFile> node).fileName) + '"',
                                                 /*isBlockScopeContainer*/ true);
                        break;
                    }
        ...
        function bindParameter(node: ParameterDeclaration) {
            if (isBindingPattern(node.name)) {
                ...
                bindDeclaration(node, SymbolFlags.FunctionScopedVariable, SymbolFlags.ParameterExcludes,
                                /*isBlockScopeContainer*/ false);
            }
            ...
            if (node.flags & NodeFlags.AccessibilityModifier && node.parent.kind === SyntaxKind.Constructor &&
                ...
                declareSymbol(classDeclaration.symbol.members, classDeclaration.symbol, node, SymbolFlags.Property,
                              SymbolFlags.PropertyExcludes);
            }
        }

        function bindPropertyOrMethodOrAccessor(node: Declaration, symbolKind: SymbolFlags, symbolExcludes: SymbolFlags,
                                                isBlockScopeContainer: boolean) {
